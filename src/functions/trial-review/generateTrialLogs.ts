 ...
 */
export async function generateTrialLogsContent(characterName: string): Promise<string | null> {
  ...
  const links = codes
    .map((code, i) => `${i + 1}. [Report ${code}](https://www.warcraftlogs.com/reports/${code})`)
    .join('\n');
