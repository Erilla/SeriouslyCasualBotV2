...
function formatApiHealthLine(label: string, summary: ServiceSummary): string {
  ...
  const successPart =
    totals.retried > 0 ? `${totals.ok} ok (${totals.retried} retried)` : `${totals.ok} ok`;

  const failureParts: string[] = [];
  const totalFailures =
    totals.rateLimited + totals.timeouts + totals.failed + totals.circuitRejected;
  if (totalFailures > 0) {
...
export default {
  data: new SlashCommandBuilder().setName('status').setDescription('Show bot health and status'),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const raiders = db
      .prepare(
        'SELECT COUNT(*) as total, COUNT(discord_user_id) as linked FROM raiders WHERE inactive_since IS NULL',
      )
    ...
    const activeTrials = db
      .prepare("SELECT COUNT(*) as count FROM trials WHERE status = 'active'")
      .get() as { count: number };
    ...
    const embed = createEmbed('Bot Status').addFields(
      ...
      {
        name: 'Last Achievements Update',
        value: formatAge(achievementsStatus?.lastRun),
        inline: true,
      },
      ...
      {
        name: '\u200B',
        value: formatApiHealthLine('Raider.io', apiSummaries.raiderio),
        inline: false,
      },
      {
        name: '\u200B',
